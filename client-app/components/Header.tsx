  ...
  return (
    <View style={styles.header}>
      <View style={styles.left}>
        {showBack ? (
          <IconButton label="Go back" icon="arrow-back-outline" onPress={() => router.back()} />
        ) : null}
        {showMenu ? (
          <IconButton label="Menu" icon="menu-outline" />
        ) : null}
        {showLogo ? (
          <Image
            source={require('@/assets/brand/nimbus-wordmark-transparent.png')}
            style={styles.logo}
            resizeMode="contain"
            accessibilityLabel="Nimbus marketplace"
          />
        ) : title ? (
      ...
      <View style={styles.actions}>
        {showSearch ? (
          <IconButton label="Search" icon="search-outline" />
        ) : null}
        {showCart ? (
          <Pressable
          ...
          >
            ...
            {itemCount > 0 ? (
              <View style={styles.count}>
                <Text style={styles.countText}>{itemCount > 9 ? '9+' : itemCount}</Text>
              </View>
            ) : null}
          </Pressable>
        ) : null}
